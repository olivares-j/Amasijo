...
base_fld  = "Logg_l{0}_s{1}/"
base_mlp  = "{0}mlp.pkl"
base_lss  = "{0}loss.png"
base_fit  = "{0}fit.png"
base_err  = "{0}error.png"
base_mad  = "{0}Logg_MAD.png"
#------------------------------------------------------------------------------------
...
# df_iso.to_csv(file_iso.replace(".dat",".csv"),index=True)
...
for num_layers in list_of_num_layers:
	...
	df.columns = ["age","Mini","Teff","target","value"]
	print(df.describe())
...
plt.legend(bbox_to_anchor=(1.01, 0.5), loc="center left")
fig.savefig(base_mad.format(dir_mlps))
plt.close()
