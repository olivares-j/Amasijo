...
class MLP_logg:
	...
	def __init__(self, 
		...
		try:
			...
			assert num_layers == 13, "Error: mismatch in number of layers!"
	...
	def __call__(self, age, mass, n_stars):
	 ...
	 5. Final linear mapping produces targets: [mass, phot_band_1, phot_band_2, ...].
		...
		targets = pt.dot(A13, self.W[13]) + self.b[13]
...
class MLP_teff:
	...
	def __call__(self, age, mass, n_stars):
	 ...
	 5. Final linear mapping produces targets: [mass, phot_band_1, phot_band_2, ...].
		...
		A15 = relu(pt.dot(A14,self.W[14]) + self.b[14])
		A16 = relu(pt.dot(A15,self.W[15]) + self.b[15])
		# A17 = relu(pt.dot(A16,self.W[16]) + self.b[16])
		# Final linear output (no activation) gives targets for mass + photometry bands
...
if __name__ == "__main__":
	...
	file_mlp_logg = dir_base + "MLPs/Logg_l13_s256/mlp.pkl"
	...
	logg = mlp_logg(age,mass,n_stars)
